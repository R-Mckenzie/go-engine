...
// This all comes from an cimgui-go example repo
// github.com/ptxmac/cimgui-go
...
func NewImguiRenderer() *imguiRenderer {
	...
	io.SetBackendFlags(io.GetBackendFlags() | imgui.ImGuiBackendFlags_RendererHasVtxOffset)
	return &renderer
}

func (renderer *imguiRenderer) createDeviceObjects() {
	// Backup GL state
	...
	shader, err := NewShader("shaders/imguiVertex.glsl", "shaders/imguiFragment.glsl")
	if err != nil {
		panic(err)
	}
...
func (renderer *imguiRenderer) createFontsTexture() {
	// Build texture atlas
	io := imgui.GetIO()
	...
	io.GetFonts().SetTexID(imgui.ImTextureID(renderer.fontTexture))

	// Restore state
	gl.BindTexture(gl.TEXTURE_2D, uint32(lastTexture))
}

func (renderer *imguiRenderer) Render(displayWidth, displayHeight, fbWidth, fbHeight float32, drawData imgui.ImDrawData) {
	// Avoid rendering when minimized, scale coordinates for retina displays (screen coordinates != framebuffer coordinates)
	if (fbWidth <= 0) || (fbHeight <= 0) {
		return
	}
	drawData.ScaleClipRects(imgui.ImVec2{
...
func (renderer *imguiRenderer) dispose() {
